  ...
  switch (r.scheme_type) {
    case 'fixed_rate':
      if (r.is_scheme_transition) {
        // мітка навмисно містить "Ставка" (без "Фасовка"/"Упаковка" окремою
        // категорією) — щоб у "Деталізації фонду" по відділу Склад це
        // рахувалось як звичайна ставка, а не губилось/потрапляло в "Допки"
        push('Ставка (відрядна до переходу)', r.piece_total);
      ...
      if (SALES_DEPTS.includes(emp.dept_code) || ORDER_DEPTS.includes(emp.dept_code)) {
        ...
        if (['rop','head','teamlead'].includes(effRole)) {
          ...
          if (effRole === 'rop' && ROP_CFG[emp.dept_code]) {
            ...
            const payout1 = rc.rate / 2;   // аванс 15-го = половина ставки (враховує персональну ставку)
            return {
              employee_id: emp.id, name: emp.name,
              dept_code: emp.dept_code, dept_name: emp.dept_name,
